...
Packet Packet::deserialize(const std::vector<uint8_t> &raw) {
    ...
    auto find_val = [&header](const std::string &key) -> std::string {
    ...
    pkt.id = find_val("id");
    if (pkt.id.empty()) pkt.id = "deadbeef";

    std::string ts = find_val("type");
    if (!ts.empty()) pkt.type = (PacketType)std::stoi(ts);

    pkt.room = find_val("room");
    pkt.target = find_val("target");

    std::string ss = find_val("seq");
    if (!ss.empty()) pkt.sequence = (uint32_t)std::stoul(ss);

    uint32_t data_start = 6 + hlen;
    if (data_start < raw.size()) {
        pkt.data_str = std::string((const char *)&raw[data_start]);
    }
...
Connection::Connection(int sockfd, const std::string &addr, uint16_t port)
    ...
    id = addr + ":" + std::to_string(port) + "-" + std::to_string(counter++);
}

Connection::~Connection() { close(); }

bool Connection::send(const std::string &data) {
    Packet pkt(PKT_DATA, data);
    return send(pkt);
}
...
bool Connection::send(Packet &pkt) {
    ...
    while (sent < (int)raw.size()) {
        int n = ::send(sockfd_, (const char *)(raw.data() + sent),
                      (int)(raw.size() - sent), 0);
...
Room::Room(const std::string &name, int max_players)
    : name(name), max_players_(max_players) {}
...
bool Room::remove(Connection &conn) {
    ...
    if (it != connections_.end()) { connections_.erase(it); return true; }
    return false;
}
...
Server::Server(uint16_t port)
    : port_(port), sockfd_(-1), running_(false), host_("0.0.0.0") {
    cpp_ensure_winsock();
}

Server::~Server() {
    stop();
    for (auto &[id, conn] : connections) delete conn;
    ...
    while (conn->is_alive() && running_) {
        ...
        if (n >= 6) {
            ...
            if (msg_len <= (uint32_t)n) {
                std::vector<uint8_t> pkt_data(buf, buf + msg_len);
                auto pkt = Packet::deserialize(pkt_data);
                if (pkt.type == PKT_PING) {
                    Packet pong(PKT_PONG);
                    conn->send(pong);
                } else if (data_cb_) {
                    data_cb_(*conn, pkt);
                }
...
Client::Client() : sockfd_(-1), connected_(false), port_(0) {
    cpp_ensure_winsock();
}
        ...
        if (n >= 6) {
            ...
            if (msg_len <= (uint32_t)n) {
                ...
                if (pkt.type == PKT_DATA) {
                    if (msg_cb_) msg_cb_(pkt.data_str);
                    if (data_cb_) data_cb_(*this, pkt);
                }
            }
        }
    }
